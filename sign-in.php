...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($email) || empty($password_raw)) {
    $message = '<div class="alert alert-danger text-white">Please enter both email and password.</div>';
  } else {
    // 2. Prepare SQL statement to fetch user by email
    ...
    if ($result->num_rows === 1) {
      ...
      if (password_verify($password_raw, $hashed_password)) {
        ...
        // **SESSION HANDLING: Set session variables**
        ...
        $message = '<div class="alert alert-danger text-white">Invalid email or password.</div>';
      }
    } else {
      $message = '<div class="alert alert-danger text-white">Invalid email or password.</div>';
    }
